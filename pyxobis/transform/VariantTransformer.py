...
class VariantTransformer:
    """
    Methods for extracting and building Variant objects from pymarc Records.
    """
    def __init__(self):
        pass
    ...
    def transform_variants(self, record):
        """
        For each field describing a variant in LaneMARCRecord record,
        build a Variant.
    ...
    def transform_variant_being(self, field):
        ...
        # Included
        # ---
        field, included = self.__extract_included_relation(field)
        ...
        # ---
        type_kwargs, type_time_or_duration_ref = tfcm.get_type_and_time_from_relator(field)
        if type_kwargs:
        ...
        # Name(s) & Qualifier(s)
        # ---
        variant_names_and_qualifiers = NameParser.parse_being_name(field)
    ...
    def transform_variant_concept(self, field):
        ...
        if field.tag == '072':
            ...
            cvb.set_type(entry_type,
                         Indexer.simple_lookup("Equivalence", CONCEPT),
                         Indexer.simple_lookup(entry_type, RELATIONSHIP))
        ...
        # Name(s) & Qualifier(s)
        # ---
        variant_names_and_qualifiers = NameParser.parse_concept_name(field)
        for variant_name_or_qualifier in variant_names_and_qualifiers:
            if isinstance(variant_name_or_qualifier, dict):
    ...
    def transform_variant_event(self, field):
        ...
        # ---
        variant_names_and_qualifiers = NameParser.parse_event_name(field)
        for variant_name_or_qualifier in variant_names_and_qualifiers:
            if isinstance(variant_name_or_qualifier, dict):
    ...
    def transform_variant_language(self, field):
        ...
        # Name(s) & Qualifier(s)
        # ---
        variant_names_and_qualifiers = NameParser.parse_language_name(field)
        for variant_name_or_qualifier in variant_names_and_qualifiers:
            if isinstance(variant_name_or_qualifier, dict):
    ...
    def transform_variant_organization(self, field):
        ...
        # Name(s) & (Pre)Qualifier(s)
        # ---
        variant_names_and_qualifiers = NameParser.parse_organization_name(field)
        for variant_name_or_qualifier in variant_names_and_qualifiers:
            if isinstance(variant_name_or_qualifier, dict):
    ...
    def transform_variant_place(self, field):
        ...
        # Name(s) & Qualifier(s)
        # ---
        variant_names_and_qualifiers = NameParser.parse_place_name(field)
        for variant_name_or_qualifier in variant_names_and_qualifiers:
            if isinstance(variant_name_or_qualifier, dict):
    ...
    def transform_variant_string(self, field):
        ...
        # Name(s) & Qualifier(s)
        # ---
        variant_names_and_qualifiers = NameParser.parse_string_name(field)
        for variant_name_or_qualifier in variant_names_and_qualifiers:
            if isinstance(variant_name_or_qualifier, dict):
    ...
    def transform_variant_time(self, field):
        ...
        if variant_is_duration:
            ...
            calendar1_kwargs, start_datestring = DateTimeParser.extract_calendar(start_datestring)
            if calendar1_kwargs:
                tvb.set_calendar1(**calendar1_kwargs)
            calendar2_kwargs, end_datestring = DateTimeParser.extract_calendar(end_datestring)
            if calendar2_kwargs:
            ...
            start_time_content_single = DateTimeParser.parse_simple(start_datestring)
            tvb.set_time_content1(start_time_content_single)
            end_time_content_single = DateTimeParser.parse_simple(end_datestring)
    ...
    def transform_variant_work_authority(self, field):
        ...
        if field.tag == '130':
            type_kwargs = type_kwargs or { 'link_title' : "Uniform title",
                        ...
                        'href_URI'   : Indexer.simple_lookup("Uniform title", RELATIONSHIP) }
        if type_kwargs:
        ...
        # Name(s) & Qualifier(s)
        # ---
        variant_names_and_qualifiers = NameParser.parse_work_authority_name(field)
        for variant_name_or_qualifier in variant_names_and_qualifiers:
            if isinstance(variant_name_or_qualifier, dict):
    ...
    def transform_variant_work_instance_or_object(self, field, element_type):
        ...
        if element_type == WORK_INST:
            ...
            name_parser = NameParser.parse_work_instance_variant_name
        else:
            vb = ObjectVariantBuilder()
            name_parser = NameParser.parse_object_variant_name
        ...
        type_kwargs, type_time_or_duration_ref = self.__get_type_and_time_from_title_field(field)
        if type_kwargs:
    ...
    def __extract_included_relation(self, field):
        """
        Extracts "included" relation from a pymarc Field representing a Variant.

        Returns the modified Field object, and a string value for the "included"
        attribute (or None if n/a), for use in a VariantBuilder.
        """
        subf_code = 'i' if field.tag in LaneMARCRecord.RELATOR_SUBF_i_TAGS else 'e'
    ...
    def __get_type_and_time_from_title_field(self, field):
        """
        ...
        if field.tag in ('246','247'):
            for code, val in field.get_subfields('f','g', with_codes=True):
                ...
                parsed = NameParser.parse_generic_qualifier(val, field['3'], field['4'])
                if isinstance(parsed, TimeRef) or isinstance(parsed, DurationRef):
